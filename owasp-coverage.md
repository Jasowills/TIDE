...
| A03 | Supply Chain | **Covered** | OSV-Scanner gate (Go + npm, `security.yml`, fails non-zero). `package-lock.json` committed; console Dockerfile installs via `npm ci`. Dependabot (`.github/dependabot.yml`). Semgrep supply-chain rules in SAST gate. |
...
| A10 | Exceptional Conditions | **Covered** | Generic `internal error` 500s; backend failures fail closed (`TestErrorContractLeaksNothing`). PG-down degrades to bounded memory buffer, never fails open (`TestPostgresDownDoesNotKillIngestion`, boot). Chaos suite: malformed/duplicate/late storms + live Redis restart rebuild (tests). Fuzzing beyond fixed fixtures: deferred, ZAP active scan covers breadth in CI. |
